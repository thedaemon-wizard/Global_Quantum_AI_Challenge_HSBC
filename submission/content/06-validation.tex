...
band-conditional false-decline rate is certified at a level the business names, with family-wise
error control, and the certificate then holds on a block never used to build it. Average
precision and ROC AUC are reported alongside but decide nothing: they are not statements about a
decision and carry no finite-sample guarantee. The corresponding failure is equally well
defined --- the admissible set comes back empty, which is what happens here at the tightest band
budget.

\textbf{Held-out validation is the acceptance test.} The certificate is selected on calibration
data, then read on a fold protected by a ledger that refuses a second distinct configuration.
Here \ClaimHFiveConfigsHolding{} of \ClaimHFiveConfigsTotal{} held their risk, the worst at
\ClaimHFiveWorstRatio{} of its budget. A PoC would run exactly this test and report it whichever
way it came out.

\textbf{Benchmarking is against a tuned baseline, replicated, with the power stated first.} The
comparator is a tuned gradient-boosted model, not a straw man, on identical rows and features,
and intervals resample entities rather than rows. Crucially, a detectable-effect ceiling is fixed \emph{before} the comparison: here
