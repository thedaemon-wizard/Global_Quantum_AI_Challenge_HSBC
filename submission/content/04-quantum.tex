...
\ClaimMpsShareFull\,\% of it, against \ClaimMpsShareBand\,\% in the band, so the gap widens
with dimension rather than capacity. Every one of the \ClaimSweepJobs{} loses at a
...
\ClaimSweepJobs{} fits diverged to the prior}, both at one seed and at two different bond
dimensions, on the sequential contraction --- the stable path. A re-run against matched seeds
reproduces both bit-identically: their best loss falls by epoch \Record{2} of \Record{30} and
rises after, where the controls improve to the last. Excluding them \emph{raises} the
ratio to \ClaimSweepSpreadRatioTrained{} --- they depressed two of the four means, so part of
the apparent capacity signal was the failures themselves.
