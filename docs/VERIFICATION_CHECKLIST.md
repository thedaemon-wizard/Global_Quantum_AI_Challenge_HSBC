...
| `[x]` | The repository URL printed in the proposal title block | `curl` unauthenticated | 404 on 2026-09-02 while private; **HTTP 200 on 2026-09-14**, when the repository went public a day ahead of the deadline. The title block now resolves for a reviewer who opens the PDF early, which is what this row was guarding |
| `[x]` | The portfolio, which is the proposal's only verification URL | fetched and read 2026-09-02 | HTTP 200. Four claims agree with §8 exactly: the sole proprietorship "filed 1 Aug 2026", Team MerQury, three merged pull requests with two bounty issues (#2242 and #749) for USD 200, and QIntern 2026. A fifth diverged on 2026-09-06 and closed again on 2026-09-14, when [D-156](decisions.md) removed the Qiskit credential from §8 altogether: the section now names none, so the portfolio's Advocate line contradicts nothing. **One difference remains, and the proposal is the one to trust** -- see §5c |
...
| `[x]` | **The Colab claim in README section 7** | simulated runtime, 2026-09-14 | Notebook fetched from `raw.githubusercontent.com` (byte-identical to the committed copy), executed from an empty stand-in for `/content` with `google.colab` importable as a PEP 420 namespace package and no repository present. It cloned itself and ran to completion: **23 of 24 cells byte-identical** to the committed outputs, the one difference being the bootstrap cell's `Cloning into ...`. Not made a test -- it needs the network and would fail for unrelated reasons. [D-176](decisions.md) |
| `[x]` | **Eighth clean-room pass, the deadline tree** | clone at `bf709ba`, 2026-09-14 | `make venv`, `smoke`, `walkthrough`, `check`, `pytest`, `submission` all exit 0. 105 claims agree, 51 scientific artefacts match, hash `a8275e8fc7666cb3`, 216 passed. **`make reproduce` deliberately skipped**: an AST comparison with docstrings stripped showed every producer bit-for-bit unchanged since the seventh pass, so refitting could only re-derive identical tables. First clone to run `summarise_screens.py` and both notebook gates -- the latter without `.[notebook]` installed. All five staged files byte-identical to the working tree. [CLEANROOM.md](CLEANROOM.md) §2h |
...
§8 links the portfolio, so a reviewer can open both. Four rows were recorded here; **one has
since closed** and the rest are not defects in this submission. All four live in files outside
...
| Yale method reach | "9/10 peaked-circuit challenges **up to 69 qubits**" | "the matrix-product-state attack is his own, merged upstream, exact to **50** qubits" | **Both, about different things, and the submission is now narrower than this row once recorded.** Nine of ten were solved by the team and the largest was 69 qubits. §8 no longer claims 60: the only run the author's own attack is evidenced on is the 50-qubit one, where the output file records `Weak=0/50` and a bit string matching the published answer exactly. [D-059](decisions.md) established that no artefact attributes the 69-qubit answer to a tensor network; the 60-qubit problem was solved by a different code path that compressed the circuit to 29 gates. Team result against personal contribution -- not a contradiction, but a reviewer reading them together may not see that |
...
| Qiskit credential | "Qiskit Advocate (Feb 2026)" | *nothing* | **No longer a divergence.** [D-156](decisions.md) removed the credential from §8: its issuer grades it "Foundational", and beside three merged upstream pull requests and hardware runs it added no evidence to a section scored on whether the team can execute. Both credentials are held and both are recorded in [CREDENTIALS.md](CREDENTIALS.md) §8 with the badge URL. A portfolio listing more than the proposal is normal; the earlier state, where each named a *different* Qiskit credential, was the problem, and it is gone. No author action |
