...
`scripts/check_claims.py --citations` gates two citation forms across the README, the protocol,
the decision log, the provenance and checklist files and every proposal source: an **entry
...
which searches by identifier, by arXiv id, and by first-author surname.
...
**One exception, deliberate.** PyTorch is pinned in the `Makefile`, not in `pyproject.toml`.
...
undo that. The pin is `torch==2.13.0` at `Makefile:44`, and the version was checked against that index rather than
