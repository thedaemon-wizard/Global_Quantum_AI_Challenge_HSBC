...
**[QM-20]** Mancilla, J. and Tagliani, T. "The Fourier Wall: Why Public Tabular Datasets Refuse
Quantum Advantage, and a Certified Recipe for Where It Lives". arXiv:2607.15815, 17 July 2026
(v1; abstract read 2026-09-14).
**The closest independent statement of this study's own method, arrived at separately.** It
argues that QML losing to tuned classical baselines on public tabular data "is a structural
property of the datasets rather than merely a limitation of current models", and operationalises
that as SPECTRA -- "a **simulator-free structural screen** followed by a decisive comparison
between a matched quantum model and five tuned classical twins". The screens in
`src/hsbcfraud/quantum/screens.py` are the same move: reject a priori on properties of the
kernel, before any downstream accuracy can launder the result.

It also names conditions for advantage where this submission names only a direction. Its claim
is that an angle-encoded model is a partial Fourier series, so advantage needs a target spectrum
"simultaneously off-grid, of interaction order at least three, high-frequency, supported by
near-independent features, and dense beyond practical enumeration" -- and its positive result is
on labels *generated by an interacting quantum process* (0.994 against 0.699), not on a natural
target. Cited for the structural argument, not for the numbers: its decisive comparison is
energy smart-meter data, not payments, and this study did not run SPECTRA.

**[QM-21]** Doosti, M., Wallden, P., Hamill, C. B., Hankache, R., Thomson Brown, O. and
Heunen, C. "A brief review of quantum machine learning techniques for financial services".
*Machine Learning: Science and Technology* **7**(2):021002, 8 April 2026. Open access,
DOI 10.1088/2632-2153/ae51df. Verified against the publisher record 2026-09-14.
**Peer-reviewed, 2026, and co-authored inside a bank** -- four authors at the Quantum Software
Lab, University of Edinburgh, two at Data Science and Innovation, NatWest Group. That makes it
the strongest available external anchor for the position section 4 takes, because it is neither
a preprint nor written by an outsider to the industry.

Its verdict on this exact application: "Most QML applications in finance remain at the
proof-of-concept stage ... however, to date, **there is no large-scale deployment of QML models
in live financial systems**." On fraud specifically it reports "**competitive** accuracy compared
to classical methods such as logistic regression, SVM, random foresting and gradient boosting"
-- competitive, never superior -- and adds that the studies are "primarily simulation-based".
