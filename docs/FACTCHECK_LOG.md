...
| 10 | README: reference ranges CP-1…13, QM-1…10, SW-1…9 | [REFERENCES.md](REFERENCES.md) ID scan | **not supported** | Actual maxima are CP-17, QM-15, SW-10; 59 entries in total, which [REFERENCE_CROSSCHECK.md](REFERENCE_CROSSCHECK.md) independently confirms. Understated by ten |
...
| 30 | "The author's working repo records different peak bitstrings for P3–P9 than the published set" — my own 要確認 | `github.com/roman-bagdasarian/Peaked-Circuits`, `data/` tree and `git log` | **not supported; withdrawn** | The repo holds two problem sets. Published answers are **2025's** (five `.txt`, committed 2026-04-09 by the owner); `Yale_Quantum_2026/` has exactly one answer file, added by the author himself. Problem names diverge from P2 (2025 `swift_rise`/`sharp_peak`; 2026 `small_bump`/`tiny_ripple`), six problems vs ten. There was no 2026 published set to disagree with |
