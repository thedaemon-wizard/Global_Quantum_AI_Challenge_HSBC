...
**Needs confirmation** (could not be settled from an artefact available here).
...
| 13 | REGULATORY_SOURCES: Needs confirmation on the SS1/23 applicability sentence | [REFERENCES.md](REFERENCES.md) RG-5 | **stale** | The defect had already been corrected in REFERENCES.md; the open marker was left behind. Closed |
...
| 15 | CREDENTIALS: the cited source path for the QIntern work | `git rev-parse` against the freeze commit in the Team A repository | **not supported** | The path named a drive copy holding documentation only, one level above the repository that carries the history. Both locators have since been removed: the repository is private, so no locator in it is checkable by a reviewer |
| 16 | CREDENTIALS: "**Every** Week-5 artefact carries `source_kind: dummy`" | a recursive count of the Week-5 source stamps → 21 dummy, 7 real | **overstated** | True of all eleven run-level stamps; six row-level entries read `"real"` (the XGBoost baseline). The distinction favours the work — the classical arm was measured — and is now stated |
| 17 | §8: the author wrote "an exact Beta-Binomial coverage harness … exact McNemar under Holm" | `git log --follow` on the coverage harness and the statistics protocol | **not supported** | Both land in a commit by a teammate. `CREDENTIALS.md` already said so, so the two documents contradicted each other; §8 now attributes them to the team |
...
| 19 | QIntern: "135 checks green at that freeze" | `git grep -h "def test_" <freeze> -- '*/tests/*.py' \| wc -l` = 135; at `HEAD` = 183 | **supported** | Exact. No `parametrize` at the freeze, so defined equals collected there. The 183 is a raw `def test_` count at `HEAD` and the Week-5 README's 182 is the collected figure, which is why `CREDENTIALS.md` §5 prefers the latter; absence of `parametrize` rules out under-counting, not over-counting. The proposal's "at that freeze" hedge is correct |
| 20 | QIntern: five seeds, three intrusion-detection corpora | the frozen v1.0 results manifest; `SEEDS = [42…46]` | **supported** | `['CICIoT2023', 'BoT-IoT', 'UNSW-NB15']`. No occurrence of "card fraud" or "payment" anywhere in the repository, so the no-extrapolation claim holds |
...
| 28 | "conformal_calibrate.py authorship is not independently confirmable" — my own Needs confirmation | a Days 16--17 handoff document, committed by the teammate | **not supported; the Needs confirmation is lifted** | The teammate's own handoff names it as the author's Day-15 calibration module and records re-running it to a six-decimal match. The bulk "Initial commit" was also **the teammate's**, not the author's, so the premise that only self-attestation remained was wrong twice. The Days 26–27 freeze is verified the same way, by a second teammate reproducing the manifest byte for byte |
| 29 | "the Week-3 drive copy is documentation only, with no scripts or tests" — my own statement | directory listing of that copy | **not supported** | True of Week 5, false of Week 3, which has `scripts/` (including `conformal_calibrate.py`) and `tests/`. Corrected |
| 30 | "The author's working repo records different peak bitstrings for P3–P9 than the published set" — my own Needs confirmation | both README tables, compared programmatically; `data/Yale_Quantum_2026/P5_soft_rise.qasm.txt` | **not supported; it is an endianness artefact** | Same nine answers in opposite bit order. P3–P9 are **exact string reversals**; P1 is a palindrome; P2 is the one row the third-party README transcribed un-reversed. The author's own committed fixture prints both orientations from one run. His copy is the submitted orientation, so the doubt ran the wrong way |
...
Rows here are numbered `1b-n`. They previously restarted at 15 and ran to 23, which collided
with section 1's rows 15--23: nine numbers each denoted two different checks, so a citation of
"row 19" was ambiguous about which round it meant.
...
| 1b-2 | The certified rule applies an in-band re-scorer `g` at threshold λ | `scripts/run_conformal.py:181`, `scripts/validate_certificate.py:90-93`, `conformal/riskcontrol.py:108` | **not supported as written** | λ is selected over, and applied to, the same full-traffic score. For the reported configuration λ = 0.0582 inside a band of [0.0320, 0.0718). Nothing is wrong mathematically; no sentence said `g = f` in this run ([D-086](decisions.md)) |
| 1b-3 | Appendix A8: "three scripts read `D_test` without the guard" | `grep -l TestFoldGuard scripts/*.py` against the scripts that read the block | **not supported** | Six read it and four are unguarded; the protocol table also listed `run_mps.py` as authorising when it never imports the guard. A test now verifies the table against the tree ([D-089](decisions.md)) |
| 1b-4 | Appendix: the first four amendments never used `D_cal` risks | `docs/protocol.md` A3 ("Measured on `D_band` and `D_cal`") and A4 ("Measured on `D_cal` at a 5 % band") | **not supported** | A3 and A4 both used calibration-set quantities. The sentence now names them and A6 is reclassified from correction to disclosure |
| 1b-5 | `make reproduce` regenerates the committed results | `grep` for each table name across `scripts/` and `src/` | **not supported for four tables** | `split_arm_baselines.csv`, `rolling_origin.csv`, `mps_seed_sweep_summary.csv` and `mps_seed_spread.csv` had no producer. Three now have one and reproduce the committed values; the fourth is disclosed ([D-089](decisions.md)) |
| 1b-6 | Proposal §4: "the maximum 1.0" RBF correlation | [`screens.csv`](../results/tables/screens.csv) | **not supported as printed** | The measurement is 0.99999975. `claims.yaml` recorded `1.0000` and YAML parsing dropped the trailing zeros, so the page asserted exactly one in a sentence about values indistinguishable to floating point |
| 1b-7 | `test_makefile_scripts_exist` enforces that referenced scripts exist | `tests/test_repo_hygiene.py` | **not supported** | It called `pytest.xfail`, which the runner counts as a pass, so it could not fail. Now an assertion |
| 1b-8 | Phase II runs from 17 November 2026 | [Program roadmap](https://quantumai.thequantuminsider.com/program/) | **supported** | Phase I closes 15 Sep 2026; review 16 Sep – 14 Nov; Phase II PoC 17 Nov 2026 – 28 Feb 2027 with AWS credits and Classiq tooling; winners announced 30 Apr 2027 |
| 1b-9 | The portal accepts five uploads and a fixed format list | HSBC challenge panel, 2026-08-31 | **supported** | "0 uploaded, 5 slots left"; PDF, PNG, JPG, WEBP, GIF, PY, JSON, JS, XLS, XLSX, CSV, DOC, DOCX. GUIDE §5 adds "File size must not exceed 20 MB" without saying per file or total; the staged set is 12 MB total and the largest file is 11 MB, so it is under the cap either way |
...
## 3. What remains Needs confirmation
...
| ~~Yale problems 3–9 peak bitstrings~~ and ~~`conformal_calibrate.py` authorship~~ | **Both closed on 2026-08-30; see rows 30 and 28 above.** The bitstring difference is an endianness artefact — the same nine answers in opposite bit order — and the authorship is attributed by a teammate's own committed handoff, which also shows the bulk "Initial commit" was the teammate's. Retained here only so a reader who remembers them as open finds the closure. The earlier text also argued that a score of 450 records nine correct submissions, which [CREDENTIALS.md](CREDENTIALS.md) retracts: 450 is consistent with six, seven, eight or nine solved |
