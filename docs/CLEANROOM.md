...
re-measured to **66.8 %** on an idle host, after [D-147](decisions.md) and [D-153](decisions.md) corrected the model the
...
the time; it is **66.8 %** since the 2026-09-11 re-measurement, [D-153](decisions.md).) Nothing was wrong with the code.
...
materially -- `band_edges` and the split-conformal order statistic consolidated into the
library, `configuration_digest` extracted, `reps` threaded to five call sites, the latency
script pointed at the shipped feature pipeline, `--seed 0` stopped collapsing to the default --
so that pass no longer covered what ships. This one clones `baf2d17` and runs README section 7
end to end. **Zero producer files changed between that commit and the final tree**, so it
covers the submitted code.

```
=== EXIT-VENV 0 ===        1m34s
=== EXIT-SMOKE 0 ===       all 9 checks accounted for
=== EXIT-WALKTHROUGH 0 === every assertion holds against the committed tables
=== EXIT-REPRODUCE 0 ===   3h28m
=== EXIT-CHECK 0 ===       102 claims agree; 51 scientific artefacts match the manifest
=== EXIT-PYTEST 0 ===      212 passed, 1 skipped
```

**Zero errors or tracebacks** in the whole run. Six logger files written, 611 to 3,234 bytes.

**The artefact count in that block is 51 and the manifest now holds 50.** Nothing was removed: `results/tables/smoke.json` moved from the scientific class to the specification class on 2026-09-12, because it records what S0-S8 asserted about *this machine* -- driver, device name, CUDA build -- and so cannot keep the byte-identity promise the scientific class makes ([D-159](decisions.md)). The block above is the run's own output and is left as it printed.
...
**Thirty-one of the thirty-five were rewritten.** The four that were not are the four designed
not to be, and each announced itself: `parity.csv` printed its skip (`qiskit_aer` is absent from
the default environment, as documented), `latency.csv` printed "NOT re-measuring latency",
`mps_seed_sweep.csv` is excluded from `reproduce`, and `mps_seed_spread.csv` is the documented
producerless exemption.

**Every built artefact is bit-identical**, by SHA-256 against the committed copies:

| Artefact | SHA-256, first 12 |
|---|---|
| `proposal.pdf` | `1fbac340a072` |
| `appendix.pdf` | `3454b8b6544d` |
| `method.png` | `16c146fbdbe3` |
| `certified_region.png` | `26ceb9dbef78` |
| `mps_h4.png` | `eeb97a5e7f02` |

### The sweep, separately

`make reproduce` excludes `make seedsweep`, so `mps_seed_sweep.csv` had never been regenerated by
any pass. It was run on 2026-09-11 in the fifth pass's clone on an idle GPU, 09:13 to 21:19, and
**all sixteen full-scale fits reproduce ROC AUC, average precision and final loss to the last
digit** -- `max |delta| = 0.000e+00` on each -- with the derived summary byte-identical. Twelve
GPU-hours of stochastic training, bit for bit. See [D-158](decisions.md).

**With that, thirty-two of the thirty-five committed tables have been regenerated from a fresh
clone and none differs in a measured value.** Three have not, and the reasons differ:

| Table | Never regenerated because |
|---|---|
| `mps_seed_spread.csv` | **it has no producer at all** -- `grep -rl mps_seed_spread scripts/ src/` returns nothing. It is the documented exemption ([PROVENANCE.md](PROVENANCE.md) section 1.4), retained as the measured evidence behind [D-038](decisions.md)'s choice of contraction, and it can never be regenerated by this repository |
| `latency.csv` | re-measurement is opt-in by design ([D-137](decisions.md)); every clean-room pass printed "NOT re-measuring latency" and preserved it. It *was* re-measured three times on this workstation ([D-153](decisions.md)), but never inside a clean room |
| `parity.csv` | the default environment lacks the `gpu-crosscheck` extra, so every pass produced four comparisons against twelve committed and took the preserve branch. It reproduces on a machine with the extra, which is how [D-154](decisions.md) found its tolerance was pinned below its own noise |

An earlier version of this sentence claimed **all** thirty-five. That was written the day the
sweep landed, and it is the failure this file exists to record: a universal quantifier asserted
