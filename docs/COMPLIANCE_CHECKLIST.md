...
| C18 | §4.1 *"Participants are encouraged to benchmark against these published results and clearly report comparison methodology"* --- the §4.1 table lists **five** | met **now**, and one ground changed. §4 previously dismissed the Deloitte/AWS precision as quoted "with no recall or base rate". The blog post was read on 2026-09-02 and **the base rate is reported**: ULB, 284,807 transactions, 492 frauds, 0.172 % — twenty times below this task's. The objection is now the accurate and stronger one, that a thresholded precision is not comparable across base rates differing by that factor. The post also gives a threshold sweep (0.87/0.89/0.92) and a classical arm (0.83/0.84/0.86) that the statement's summary omits ([D-129](decisions.md) **Scope, stated rather than left implicit.** Two of the five are engaged in §4 on their own terms: Karimi et al. (row 1, the VQC) and Deloitte + AWS (row 4, the hybrid QNN). The other three are recorded in [REFERENCES.md](REFERENCES.md) §7 as consulted and not relied upon, each with the reason in the source's own words --- Innan (row 2): F1 = 0.98 on 200 rows of BankSim, balanced 100/100, four features, no classical baseline; Ubale (row 3): a pipeline that "clean[s], encode[s], **balance[s]**, and normalize[s]" against a conventional LSTM; Grossi (row 5): a balanced ~2,500-row subsample of proprietary data, and the paper's own noisy-simulation row is 0.55 +/- 0.10 against the 0.78 statevector row. **None of the five reports a result on a full-scale unbalanced card-fraud test set against a tuned gradient-boosted baseline**, which is what makes them incomparable rather than unflattering. The like-for-like benchmark this row is really met by is §3's IEEE-CIS leaderboard comparison, which reports its methodology in full. See [D-172](decisions.md). |
...
| C24 | §5.3 Hardware: error mitigation, and DM1 for noise-aware prototyping | not applicable, and adjudicated rather than skipped | The error-mitigation bullet sits in a list introduced by "Teams using hardware are encouraged to", so it is conditioned on hardware use; DM1 is offered to "prototype noise-aware circuits **before** hardware execution", and there is none here to precede. **Corrected 2026-09-14; the earlier reasoning here was backwards.** This cell used to argue that a noise study "would price the noise sensitivity of a method the screens rejected on noiseless grounds, which cannot change a conclusion". That is not what `screens.csv` shows. Depolarizing noise drives a fidelity Gram toward the identity and so drives $r_{\mathrm{eff}}$ toward 1 — and **92 of 120 configurations fail conditioning by sitting *below* the band, not above it** (range 0.00334–0.13191, none above 0.35). Noise would move those *toward* the conditioning band, not away. The correct argument is the other screen: **distinctness is what binds, and nothing passed it** — 0 of 120, at every bandwidth. Whether noise changes RBF-distinguishability is **untested here** and is not claimed either way. So the requirement is still not applicable — the bullet is conditioned on hardware use ("Teams using hardware are encouraged to"), and DM1 is offered to prototype "**before** hardware execution", which there is none here to precede — but it is not applicable for the reason in [D-168](decisions.md), not for the reason this cell gave. The one mandatory clause in that block is the sample count, which is C14 |
...
| C28 | §4.2 Robustness: "performance consistency across different **fraud types** and data partitions" | **partitions met; fraud types not answerable on this dataset, and that is stated rather than left blank** | The partition half is the study's central result: one model over three splits of the same file, five seeds each, with a stratified split reporting 62 % more average precision than a forward holdout (§3). The fraud-type half cannot be answered here — IEEE-CIS labels `isFraud` as a bare 0/1 with **no type taxonomy**, and the label is a propagated chargeback flag rather than a categorised fraud (PROVENANCE §1.1). The nearest available proxy is `ProductCD`, five product categories, which is a transaction type and not a fraud type. Reported as unanswerable rather than answered with a proxy that would read as more than it is |
