...
| P8 | Git history carries no assistant attribution | met | No `Co-authored-by` trailer anywhere in **58** commits, re-counted 2026-09-02. The row previously said "two human authors" at 32 commits; both figures were stale. `git log` shows **three author strings, all the same person** -- `Amon Koike <amon.koike@daemons.jp>`, `a-koike <amon06251994@gmail.com>` and `thedamon-wizard <amon.koike@daemons.jp>`, the last carrying a typo in the name. ⚠ **要確認**: a reviewer reading the history sees three contributors on a submission that says sole author. Setting `user.name` and `user.email` consistently before 2026-09-15 would remove the discrepancy; rewriting history is not recommended |
