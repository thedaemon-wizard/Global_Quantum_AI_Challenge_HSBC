...
| `[x]` | Coverage is judged against the exact Beta-Binomial law, not against `rate <= alpha` | `.venv/bin/python scripts/run_coverage_arms.py`, which rebuilds both tables byte-identically. This row was unticked because **no command decided it**: `make conformal` had been named here in error -- `run_conformal.py` writes `coverage.csv`, a four-row table with no `arm` column -- and nothing in the tree wrote `coverage_by_arm.csv` or `coverage_by_arm_seeds.csv`, so the property held of the committed files with no way for a reviewer to rebuild them. The producer now derives every row from the per-seed score files through the same `split_conformal_coverage` the certificate itself uses, and `TABLES_WHOSE_PRODUCER_IS_MISSING` is empty | `coverage_by_arm.csv` column `finite_sample_ok` |
...
| `[x]` | `D_test` was never used for selection: one authorised configuration, and the guard raises on a second distinct one | `test_access.json` records **one configuration digest**, which is the property the guard enforces -- it raises on a second *distinct* configuration, not on a second run. The `evaluations` counter beside it increments on every reproduction ([D-149](decisions.md)), so it is deliberately not quoted here: this row said `evaluations: 1` from 2026-08-30 until 2026-09-12, by which time the file read 6. Amendment A8 lists the scripts that read the block for reporting outside the guard | `results/tables/test_access.json` |
...
| `[x]` | Every uploaded file's format is confirmed against the live portal form, per slot | five slots; PDF, PDF, CSV, **CSV**, PNG -- the fourth was `HSBC-riskcontrol.py` until 2026-09-14, when the portal refused the `.py` despite listing PY, and [D-171](decisions.md) replaced it with `HSBC-attribution-examples.csv`. Re-verified in a browser 2026-08-31 against the HSBC submission panel, which reports "0 uploaded, 5 slots left" and lists PDF, PNG, JPG, WEBP, GIF, PY, JSON, JS, XLS, XLSX, CSV, DOC, DOCX |
...
| `[x]` | **Repository is public before review** --- the proposal's title block prints its URL, which 404s while the repo is private | **Done 2026-09-14**, a day early. The GitHub API reports `"private": false, "visibility": "public"` and an unauthenticated fetch of `README.md` returns HTTP 200 |
...
| `[x]` | Repository is public | **Done 2026-09-14.** Verified unauthenticated, not from a signed-in browser session, which is the only check that distinguishes public from "visible to me" |
