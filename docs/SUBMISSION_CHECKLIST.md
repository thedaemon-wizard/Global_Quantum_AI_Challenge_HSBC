...
| `[x]` | Each of the challenge statement's four Expected Outcomes is answered by a named artefact a reviewer can open | per-transaction probabilities and binary predictions: `HSBC-predictions.csv`; feature attribution: proposal §5 and `attribution_examples.csv`; classical-baseline comparison: §3; quantum encoding and circuit design: §4 and §7. Checked against the objective and outcomes shown on the portal's own challenge panel, 2026-08-31 |
| `[x]` | No uploaded file exceeds the stated size cap | largest is `HSBC-predictions.csv` at 11 MB against a 20 MB limit; the guidelines state the cap per file |
