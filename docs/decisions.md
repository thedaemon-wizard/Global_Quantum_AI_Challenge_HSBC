...
<a id="d-158"></a>
### D-158 The sweep no clean room had touched reproduces to the last digit

`make reproduce` excludes `make seedsweep` -- sixteen full-scale tensor-network fits -- so
`mps_seed_sweep.csv` was the one committed table no clean-room pass had ever regenerated. It
backs `SweepBestAp` directly and, through the summary table, the seed-spread and capacity figures
section 4 argues from. It was run on 2026-09-11 in the clean-room clone at `41c52f7`, on an idle
GPU, 09:13 to 21:19.

**Every fit reproduces exactly.** ROC AUC, average precision and final loss all give
`max |delta| = 0.000e+00` across all sixteen, and `mps_seed_sweep_summary.csv` is byte-identical.
Twelve GPU-hours of stochastic training, bit-for-bit.

**The committed table predated its producer by four columns**, not the one that first prompted
this. It lacked `loss_initial`, `loss_min`, `epoch_of_min` **and** `gpu_contended`; the current
`run_seed_sweep.py` writes all four. So the committed sweep could not have been regenerated by
the shipped code under any GPU condition -- a schema difference, invisible until something tried.
The regeneration is now the committed table.

**It supersedes a caveat rather than merely explaining one.** The diverged-fit diagnostic in
`RESULTS.md` -- initial loss, best loss and the epoch it turned, for four jobs -- came from a
*contended* re-run, and carried a note saying its `fit_seconds` must not be quoted. Those three
columns are now in the main table from a clean run, reproducing all four rows exactly: AUC 0.798
and 0.533 at $\chi = 8$, 0.801 and 0.488 at $\chi = 32$, turning at epochs 30, 2, 30 and 1. The
note is retired and the diagnostic is sourced from the committed table.

**The timings moved, and the direction says something about the old ones.** Per-fit wall clock
was 2747--3145 s committed and is 2685--2783 s on the idle run: both lower and far tighter, 98 s
of spread against 398 s. The earlier measurement was taken under mild contention. Total sweep
cost is **12.1 GPU-hours**, not 13.0, and every document quoting the old figures is updated --
`README.md`, `ENVIRONMENT.md`, `RESULTS.md` and `CLEANROOM.md`.

**What this does and does not establish.** It establishes that the arm's full-scale numbers are
reproducible from a fresh clone by the shipped code, which is the strongest statement available
